    ...
    axios
      ...
      .then((response) => {
        ...
        for (let i = 0; i < response.data.genres.length; i++)
          this.idfilgenres[i] = response.data.genres[i].id;
        for (let i = 0; i < response.data.countries.length; i++)
          this.idfilcountries[i] = response.data.countries[i].id;
      })
    ...
    shareData(secPartApi, paramsApi, routerName, routerParams) {
      if (secPartApi == this.FiltersAPI) {
        if (this.filgenres != null && this.filgenres.length != 0) {
          paramsApi.genre = [];
          for (let i = 0; i < this.filgenres.length; i++)
            paramsApi.genre[i] = this.filgenres[i].id;
        }
        if (this.filcountries != null && this.filcountries.length != 0) {
          paramsApi.country = [];
          for (let i = 0; i < this.filcountries.length; i++)
            paramsApi.country[i] = this.filcountries[i].id;
        }
